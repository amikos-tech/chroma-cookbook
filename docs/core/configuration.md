...
=== "Docker"
    ...
    ```bash
    docker run -d --rm --name chromadb -v ./chroma:/chroma/chroma -e IS_PERSISTENT=TRUE chromadb/chroma:0.5.20
    ```
...
In the current Chroma version (as of time or writing `0.5.20`) the only type of telemetry supported are traces.
